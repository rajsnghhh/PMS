...
export class EditCompanyComponent implements OnInit {
  ...
  getData(id:string) {
    this.apiservice.getCompanyDetails(this.localStorageData.organisation_details[0].id,id).subscribe(data => {
      this.companyList = data.results;
      this.companyname =  this.companyList.find((e: { id: string | null; }) => e.id == id).name;
  ...
  editCompany() {
    if (this.editcompanyForm.valid) {
      ...
      let id = JSON.parse(this.datasharedservice.getLocalData('company_id'));
      this.apiservice.editCompany(body, id, this.localStorageData.organisation_details[0].id).subscribe((res: any) => {
        if(res.status == 400) {
