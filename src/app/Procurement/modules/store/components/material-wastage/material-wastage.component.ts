...
@Component({
  selector: 'app-material-wastage',
  templateUrl: './material-wastage.component.html',
  styleUrls: [
    './material-wastage.component.scss',
    '../../../../../../assets/scss/scrollableTable.scss',
  ]
})
export class MaterialWastageComponent {
  ...
  ngOnInit(): void {
    ...
    this.getmasterList()
    this.getItems()
    this.getMaterialWastageList();

    this.addCanvas = new window.bootstrap.Offcanvas(
      document.getElementById('offcanvasRightLabeladdrole')
    );

    if(this.router.url=='/pms/store/material-wastage/add'){
      this.addCanvas.show();
    }
  ...
  getUserDetails() {
    this.apiservice.gettenderUserpermission().subscribe(data => {
      ...
      if (rolesArray.includes('procurement-material-wastage-approver')) {
        this.isMW_approver = true;
      }
    })
  }
  ...
  mwCheckboxIds :any = []
  onMaterialWastagerCheckboxChange(event: any, itemId: number) {
    if (event.target.checked) {
      this.mwCheckboxIds.push(itemId);
    } else {
      const index = this.mwCheckboxIds.indexOf(itemId);
      if (index !== -1) {
        this.mwCheckboxIds.splice(index, 1);
      }
    }
  ...
  checkSubmit(){
    let req = []
    for(let i=0;i<this.mwCheckboxIds.length;i++) {
      req.push({
        "id": this.mwCheckboxIds[i],
        "status": "approved",
      })
    }
    ...
    params.set('type', 'material-wastage');
  ...
  initMaterialWastageAdvancedSearchForm() {
    ...
    this.materialWastageAdvancedSearchFormValue.site = this.localStorageData.site_data.id
  }
  ...
  getMaterialWastageList() {
    ...
    this.procurementApiSevice.getMaterialWastageDetails(params).subscribe(data => {
      this.paginationservice.setTotalItemData(data.count);
      this.materialWastageList = data.results;      
  ...
  deleteMaterialWastage(){
    ...
    this.procurementApiSevice.deleteMaterialWastage(params).subscribe((res: any) => {
