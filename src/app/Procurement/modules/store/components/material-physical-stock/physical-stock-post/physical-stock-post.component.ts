...
@Component({
  selector: 'app-physical-stock-post',
  templateUrl: './physical-stock-post.component.html',
  styleUrls: ['./physical-stock-post.component.scss',
    '../../../../../../../assets/scss/scrollableTable.scss',
    '../../../../../../../assets/scss/tableactionButton.scss']
})
export class PhysicalStockPostComponent implements OnInit {

  localStorageData: any;
  selectItemGroup: any;
  allItemGroup: any = []
  requestPostArray: any = []
  addUser: any = {
    itemgroup: '',
    item: '',
    fromDate: '',
  ...
  onSearch() {
    ...
    if (this.addUser.postStatus) {
      req.set('status', this.addUser.postStatus);
    }
  ...
  changeCheckbox(event: any, id: any) {
    if (event.target.checked) {
      let obj = {
        id: id,
        status: 'post'
      }
      this.requestPostArray.push(obj);
    } else {
      const objWithIdIndex = this.requestPostArray.findIndex((val: any) => val.id === id);

      if (objWithIdIndex > -1) {
        this.requestPostArray.splice(objWithIdIndex, 1);
      }
    }
  }
  ...
  postPhysicalStock(){
    let params = new URLSearchParams();
    params.set('organization_id', this.localStorageData.organisation_details[0].id);
    params.set('type', 'physical-stock');

    this.procurementApiService.updateProcurementSatatus(params,this.requestPostArray).subscribe(data => {
      this.toastrService.success('Physical Stock Posted Successfully', '', {
        timeOut: 2000,
      });
      this.requestPostArray=[];
      this.getPhysicalStock();
    })
  }
