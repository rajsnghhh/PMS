...
@Component({
  selector: 'app-mr-update-status',
  templateUrl: './mr-update-status.component.html',
  styleUrls: ['./mr-update-status.component.scss']
})
export class MrUpdateStatusComponent implements OnInit {
  ...
  onSubmit() {
    ...
    for (let i = 0; i < this.form.requested_items.length; i++) {
      ...
      if (this.form.requested_items[i].status == 'approved') {
        reqStatus.push(this.form.requested_items[i].status)
      }
    }

    this.createIndentData.indent_items = this.form.requested_items;
    this.createIndentData?.indent_items?.forEach((x: any) => {
      x.quantity = x.quantity_unit
      x.status='pending'   
      delete x.id
      delete x.created_at
      delete x.created_by_details
      delete x.created_by
      delete x.updated_at
      delete x.updated_by
    ...
    this.procurementApiService.updateProcurementSatatus(params, req).subscribe(data => {
      ...
      if (reqStatus.includes('approved')) {
        this.procurementApiService.addProcurementMRIndent(this.createIndentData).subscribe(
          (val) => {

            let query = new URLSearchParams();
            query.set('method', 'edit');
            query.set('indent_id', val.results.Data.id);
            query.set('mr_id', this.createIndentData.material_request);

             this.procurementApiService.updateIndentAttachment(query).subscribe(data=>{
              this.toastrService.success('Indent Created Successfully!', '', {
                timeOut: 2000,
              });
              this.backtolist()
             })
  ...
  indentform(value: any) {
    this.createIndentData.department = value.department;
    this.createIndentData.financialyear = value.financialyear;
    this.createIndentData.line_in_bottom = value.line_in_bottom;
    this.createIndentData.organization = value.organization;
    this.createIndentData.project = value.project;
    this.createIndentData.site = value.site;
    this.createIndentData.store = value.store;
    this.createIndentData.remarks = value.remarks;
    this.createIndentData.location = value.location;
    this.createIndentData.material_request = value.id;
    this.createIndentData.requested_by = value.requested_by;
    this.createIndentData.date = this.currentDate;
    this.createIndentData.time = this.currentTime;
  }
  ...
  onCheckboxChange(event: any, itemId: number) {
    ...
    for (let i = 0; i < this.form.requested_items.length; i++) {
      this.form.requested_items[i].sanctioned_quantity = this.form.requested_items[i].quantity_unit
    }
